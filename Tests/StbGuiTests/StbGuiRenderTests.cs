...
public class StbGuiRenderTests : StbGuiTestsBase
{
    ...
    public void TestRenderButton()
    {
        ...
        Assert.Equal(
            [
                new StbGui.stbg_render_command() { type = StbGui.STBG_RENDER_COMMAND_TYPE.BORDER, size = 1, bounds = { top_left = { x = 0, y = 0}, bottom_right = { x = 12, y = 5}}, color = StbGui.STBG_COLOR_BLUE, background_color = StbGui.STBG_COLOR_GRAY },
                new StbGui.stbg_render_command() { type = StbGui.STBG_RENDER_COMMAND_TYPE.TEXT, bounds = { top_left = { x = 1, y = 1}, bottom_right = { x = 10, y = 3}}, text = { text = "Button 1".AsMemory(), font_id = 1, style = { size = 1, color = StbGui.STBG_COLOR_WHITE} } },
            ],
            render_commands
        );
    }
